...
for k in range(2):
    ...
    #           + '\\Absolute_sensitivity_090819_FG\\SSA_' + str(i) + str(k + 1) + '.csv') as a:
        ...
        data['SSA_' + str('{:02d}'.format(k+1)) + '_540'] = np.reshape(np.array(df), (-1, 2))

    data['SSA_' + str('{:02d}'.format(k+1)) + '_540'] = np.array(df)
    data['SSA_' + str('{:02d}'.format(k+1)) + 'ref'] = data['SSA_' + str('{:02d}'.format(k+1)) + '_540'][370:380, 1].max()
...
data['SSA_noise'] = data['SSA_01_noise'][370:380, 1].max()
...
for k in range(2):
    ...
    # with open('C:\\Users\\uqfgotar\\Documents\\Magnetometry\\Sensitivity_calculations\\254_4\\9thAug'
    #           + '\\Absolute_sensitivity_090819_FG\\TRACE' + str('{:02d}'.format(k+1)) + '.csv') as a:
    ...
    data['S_NN_' + str(k + 1)] = np.power(10, np.divide(data['SSA_' + str('{:02d}'.format(k+1)) + '_540'][:, 1], 10))
print(data['TRACE1'].shape)
########################################################################################################################
'''Calculate the Sensitivity in function of frequency'''
ymax = np.zeros(2)
for k in range(2):
    data['Bmin' + str(k + 1)] = np.sqrt(np.divide(data['S_NN_' + str(k+1)], data['S21_' + str(k + 1)])) * B_ref
    data['Bmin' + str(k + 1)] = np.multiply(
        np.divide(data['Bmin' + str(k + 1)], np.sqrt(np.multiply(data['SNR_' + str('{:02d}'.format(k+1))], RBW))), 1e9)
    data['Bmin_min' + str(k + 1)] = data['Bmin' + str(k + 1)].min()


########################################################################################################################
'''SAVE DICTIONARY'''
pickle_out = open("sensitivity_high_freq_NA.pickle", "wb")
