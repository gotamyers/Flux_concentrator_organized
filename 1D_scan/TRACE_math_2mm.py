...
'''Read dictionary'''
pickle_in = open("1d_scan_2mm.pickle", "rb")
...
#     data2['TRACE' + str("{:02d}".format(k+1)) + '_gain'] = np.subtract(data2['TRACE' + str("{:02d}".format(k+1))][:, 1], data2['TRACE27'][:, 1])
for k in range(27):
    data2['TRACE' + str("{:02d}".format(k+1)) + '_gain'] = data2['TRACE' + str("{:02d}".format(k+1))][:, 1]

df_temp = np.zeros(shape=(751, 27))
for k in range(27):
...
'''SAVE DICTIONARY'''
pickle_out = open("1d_scan_2mm.pickle2", "wb")
